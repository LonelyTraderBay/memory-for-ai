...
- **Structural vs textual**: `CALLS` edges come from type-aware resolution (Hybrid LSP). A grep hit tells you a name appears; a `CALLS` edge records a resolved invocation. Traces are exact over stored edges, but do not prove that every source relationship was resolved or modeled: callback registrations (for example, JSX event props), partial parses, and unsupported constructs can be absent. Check coverage and source before treating an empty trace as proof that a symbol is unused.
...
```
1. list_projects                          → select the exact project for this working repo
2. index_repository(repo_path=...)        → only if missing or stale; wait for completion
3. search_graph / trace_path              → answer the task's structural question
4. get_graph_schema                       → only before writing Cypher
5. get_architecture                       → only when the task needs a project overview
```
...
| Tool | What it does · key parameters |
|---|---|
| `search_graph` | **Primary discovery tool for code structure** (definitions, callers, dependencies); use grep/`search_code` for literal strings, error text, config values, and non-code files. Three independent modes, combinable: (1) `query="update settings"` — BM25 full-text, camelCase split (`updateCloudClient` → update, cloud, client), structural boosting (Functions/Methods +10, Routes +8, Classes/Interfaces +5; noise labels filtered); (2) `name_pattern=".*regex.*"` exact pattern (ignored when `query` present); (3) `semantic_query=["send","publish"]` — vector cosine search bridging vocabulary gaps (**array of strings, not one string**; requires moderate/full index; results in `semantic_results`). Filters: `label`, `qn_pattern`, `file_pattern`, `relationship`, `min_degree`/`max_degree`, `exclude_entry_points`, `include_connected`. Start focused searches with `limit=10`; use `total`/`has_more` and paginate only when completeness matters (`limit` defaults to 50). `fields=["complexity","signature","docstring",…]` adds property columns; `detail="ids"` = cheapest bare-qn enumeration; `format="json"` = same model as JSON. |
...
| Tool | What it does · key parameters |
|---|---|
| `trace_path` | Use for callers/dependencies/impact/data flow represented by graph edges, then verify source where completeness matters. `function_name` + `project` required. `direction`: `inbound` (callers), `outbound` (callees), `both` (default). `depth` default 3 (1–5 useful). `mode`: `calls` (default), `data_flow` (value propagation with arg expressions at each hop; `parameter_name` scopes it), `cross_service` (through Route nodes + `CROSS_*` edges to other services). Response: tree rows grouped by qn-prefix; **`callers_total`/`callees_total` are exact transitive counts in the stored graph**, not proof that all source relationships were modeled. Empty traces can miss callback registrations (such as JSX event props) or partial/unsupported parses; check `index_status`, `check_index_coverage`, and source before negative/exhaustive claims. `risk_labels=true` adds CRITICAL/HIGH/MEDIUM/LOW by hop distance. Test files are excluded unless `include_tests=true`. `include_evidence=true` adds per-hop resolution strategy (`lsp` / `language_rule` / `heuristic` / `unresolved`) + confidence — use it to judge whether to trust an edge. Paginate with the `next` cursor passed back as `cursor` (all other args identical). |
| `detect_changes` | **Blast radius of a git diff over indexed relationships.** `base_branch` (default `main`) or `since` (any ref/tag, e.g. `HEAD~5`, `v0.5.0` → diffs `<ref>...HEAD`). Resolves changed files to the symbols they define, then one traversal to the stored transitive impact set. `scope`: `impact` (default) or `files` (no traversal). `direction`: `inbound` (default — what your change may break, i.e. transitive callers), `outbound` (what the change depends on), `both`. `impacted_total` is exact for the graph result; `impacted_modules` rolls up those rows. Seeds (the changed symbols themselves) are excluded from `impacted`. As with `trace_path`, check coverage and source before treating the result as exhaustive. |
...
| `delete_symbol` 🌱 | Delete a symbol's source with a graph-based usage check. **Refused while the symbol has modeled direct callers** (CALLS/HTTP_CALLS/ASYNC_CALLS) unless `force=true` — the refusal lists them; deleting a container with nested symbols also requires `force=true`. Dry-run plan shows removed lines, modeled callers, and **orphan candidates** (callees that become dead code). Same pre-write guards, backup, and incremental re-index as `edit_symbol`. The guard is not proof that no source reference exists: verify coverage and search source before deletion, especially for callback registrations such as JSX event props. Workflow: `trace_path(direction="inbound")` to review modeled caller edges → inspect source/coverage → `delete_symbol` (review plan) → apply only after the plan is complete. |
...
| What breaks if I change these files? | `detect_changes(since="HEAD~3", direction="inbound")` (committed work) or `trace_path` inbound per symbol (uncommitted idea) | `impacted_modules` is a complete rollup of the indexed impact rows even when rows truncate; verify coverage and source before treating it as exhaustive. |
...
| Delete dead code safely | Check coverage and search source → `delete_symbol(qualified_name=…)` → review plan (modeled callers + orphan candidates) → re-run with `dry_run=false` | Refusal checks modeled graph callers, not every possible source reference; verify callback registrations and unresolved references before applying. |
