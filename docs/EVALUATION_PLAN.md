...
Measure how well `memory-for-ai`'s structured knowledge-graph queries answer real developer
...
for each language L in order (skip if manifest says done):
  ...
  8. DELETE L's index from ~/.cache/memory-for-ai so the next language starts clean,
...
for lang in $ALL_LANGS; do                      # ALL_LANGS = full 159-name list (§2 / §8)
  ...
  t0=$(now_ms)
  scripts/benchmark-index.sh ~/.local/bin/memory-for-ai "$lang" /tmp/bench/"$lang" /tmp/eval-results
...
#### Challenge Review: EVALUATION_PLAN.md — memory-for-ai 159-Language Benchmark
