...
src/
  ...
  mcp/              MCP server (JSON-RPC 2.0 over stdio, 23 tools)
...
Check [issues labeled `good first issue`](https://github.com/LonelyTraderBay/memory-for-ai/labels/good%20first%20issue) for beginner-friendly tasks with clear scope and guidance.
...
existing build paths. On Windows use `npm.cmd` and `SANITIZE=` when the runtime
