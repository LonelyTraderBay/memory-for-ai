...
#
# Every memory-for-ai 0.9.x binary in the field asks for
# memory-for-ai-ui-<platform>.<ext> when its (now-removed) variant chooser
...
gh release download "$VERSION" --dir "$work" --repo "$REPOSITORY" \
  --pattern 'memory-for-ai-*.tar.gz' --pattern 'memory-for-ai-*.zip'

shopt -s nullglob
aliases=()
for archive in "$work"/memory-for-ai-*; do
  base="$(basename "$archive")"
  case "$base" in
  memory-for-ai-ui-*) continue ;;
  esac
  alias_path="$work/${base/memory-for-ai-/memory-for-ai-ui-}"
