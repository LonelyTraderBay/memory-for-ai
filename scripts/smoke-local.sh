...
#   <binary>  product binary to smoke (e.g. build/c/memory-for-ai)
...
cp "$BINARY" "$FIXTURE_DIR/memory-for-ai"
if [ -n "$ARTIFACT_DIR" ]; then
...
EXPECTED_ARTIFACT="memory-for-ai-${OS}-${ARCH}.tar.gz"
tar -czf "$FIXTURE_DIR/$EXPECTED_ARTIFACT" -C "$FIXTURE_DIR" \
    memory-for-ai LICENSE install.sh THIRD_PARTY_NOTICES.md
...
if [ "$OS" = "linux" ]; then
    cp "$FIXTURE_DIR/$EXPECTED_ARTIFACT" \
        "$FIXTURE_DIR/memory-for-ai-${OS}-${ARCH}-portable.tar.gz"
fi
