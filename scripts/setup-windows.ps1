# memory-for-ai setup script (Windows)
...
$BinaryName = "memory-for-ai"
$InstallDir = Join-Path $env:LOCALAPPDATA "memory-for-ai"
...
function Configure-ClaudeCode($McpConfig) {
    Write-Host ""
    $answer = Read-Host "Configure Claude Code to use memory-for-ai? [y/N]"
    ...
    if ($answer -match '^[Yy]$') {
        ...
        $settings["mcpServers"]["memory-for-ai"] = $McpConfig
        ...
        Write-Host ""
        $snippet = @{ mcpServers = @{ "memory-for-ai" = $McpConfig } }
...
Write-Host ""
Write-Host "memory-for-ai installer (Windows)" -ForegroundColor White
Write-Host ""
...
if ($FromSource) {
    ...
    # Clone or update
    Write-Host ""
    $sourceDir = "/home/$wslUser/.local/share/memory-for-ai"
    try {
    ...
    Invoke-WSL "mkdir -p /home/$wslUser/.local/bin && cd $sourceDir && scripts/build.sh && cp build/c/memory-for-ai $wslBinaryPath"
    ...
    Write-Host "    wsl.exe -- rm -rf ~/.cache/memory-for-ai/"
    ...
    $release = Invoke-RestMethod -Uri $releaseUrl -Headers @{ "User-Agent" = "memory-for-ai-setup" }
    ...
    $asset = "memory-for-ai-windows-amd64.zip"
    ...
    Write-Host "    Remove-Item -Recurse -Force `"$env:LOCALAPPDATA\memory-for-ai`"  # graph database"
}
