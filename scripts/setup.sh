...
# memory-for-ai setup script (macOS + Linux)
...
SOURCE_DIR="$HOME/.local/share/memory-for-ai"
...
download_binary() {
    ...
    local asset="memory-for-ai-${platform}.tar.gz"
...
build_from_source() {
    ...
    (cd "$SOURCE_DIR" && scripts/build.sh && cp build/c/memory-for-ai "${INSTALL_DIR}/${BINARY_NAME}")
...
configure_claude() {
    ...
    printf "%s" "${BOLD}Configure Claude Code to use memory-for-ai? [y/N] ${RESET}"
    read -r answer
    if [[ ! "$answer" =~ ^[Yy]$ ]]; then
        echo ""
...
JSONEOF
    ...
    if command -v jq &>/dev/null; then
        ...
        if [ -f "$settings_file" ]; then
            local tmp
            tmp=$(mktemp)
            jq --argjson entry "$mcp_entry" '.mcpServers["memory-for-ai"] = $entry' "$settings_file" > "$tmp"
            mv "$tmp" "$settings_file"
        else
            echo "{}" | jq --argjson entry "$mcp_entry" '.mcpServers["memory-for-ai"] = $entry' > "$settings_file"
        fi
...
if os.path.exists(path):
    with open(path) as f:
        data = json.load(f)
data.setdefault('mcpServers', {})['memory-for-ai'] = json.loads('$mcp_entry')
...
echo ""
echo "${BOLD}memory-for-ai installer${RESET}"
echo ""
...
info "  rm -rf ~/.cache/memory-for-ai/  # graph database"
