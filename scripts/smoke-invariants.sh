...
#
# A comprehensive, fast, portable smoke battery for the memory-for-ai
...
#
# Usage:
#   scripts/smoke-invariants.sh <binary>        # e.g. build/c/memory-for-ai(.exe)
#
# Portability notes:
...
inv_help() {
    ...
    if printf '%s' "$RB_OUT" | grep -qiE 'usage|memory-for-ai'; then
        pass "help"
    else
...
EXPECTED_TOOLS="index_repository search_graph query_graph trace_path get_code_snippet delete_symbol rename_symbol move_symbol undo_edit get_graph_schema edit_symbol compare_graphs get_architecture search_code get_code_actions list_projects delete_project index_status check_index_coverage detect_changes manage_adr ingest_traces get_runtime_traces"
