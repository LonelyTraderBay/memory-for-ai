...
CFG_WATCH=$(MFA_CACHE_DIR="$CFG_HOME" "$BINARY" config get auto_watch 2>/dev/null || true)
...
if MFA_CACHE_DIR="$CFG_HOME" "$BINARY" config get totally_bogus_key >/dev/null 2>&1; then
...
if command -v pgrep &>/dev/null && [ "$(uname)" != "MINGW64_NT" ] 2>/dev/null; then
  ...
  sleep 1
  RESIDUAL=$(pgrep -f "memory-for-ai.*cli" 2>/dev/null | wc -l | tr -d ' \n' || echo "0")
  RESIDUAL="${RESIDUAL:-0}"
  if [ "$RESIDUAL" -gt 0 ]; then
    echo "WARNING: $RESIDUAL residual memory-for-ai process(es) found"
  else
...
mcp_dump_diagnostics() {
  ...
  local cache="${MFA_CACHE_DIR:-$HOME/.cache/memory-for-ai}"
  for log in memory-for-ai-daemon.log daemon-conflicts.ndjson; do
...
DRYRUN_CACHE="$DRYRUN_HOME/.cache/memory-for-ai"
...
copy_smoke_binary "$INSTALL_DIR/memory-for-ai"
chmod 755 "$INSTALL_DIR/memory-for-ai"

# Verify installed binary works
INSTALLED_VER=$("$INSTALL_DIR/memory-for-ai" --version 2>&1)
...
copy_smoke_binary "$REPLACE_DIR/smoke-memory-for-ai"

# 3. Simulate cbm_replace_binary: unlink old, copy new
rm -f "$INSTALL_DIR/memory-for-ai"
cp "$REPLACE_DIR/smoke-memory-for-ai" "$INSTALL_DIR/memory-for-ai"
chmod 755 "$INSTALL_DIR/memory-for-ai"

# 4. Verify replaced binary works
REPLACED_VER=$("$INSTALL_DIR/memory-for-ai" --version 2>&1)
...
chmod 444 "$INSTALL_DIR/memory-for-ai"
rm -f "$INSTALL_DIR/memory-for-ai"
cp "$REPLACE_DIR/smoke-memory-for-ai" "$INSTALL_DIR/memory-for-ai"
chmod 755 "$INSTALL_DIR/memory-for-ai"
READONLY_VER=$("$INSTALL_DIR/memory-for-ai" --version 2>&1)
...
DEVIN_SKILL="$DEVIN_DIR/skills/memory-for-ai/SKILL.md"
CODEBUDDY_MCP="$FAKE_HOME/.codebuddy/.mcp.json"
CODEBUDDY_INSTRUCTIONS="$FAKE_HOME/.codebuddy/CODEBUDDY.md"
CODEBUDDY_SKILL="$FAKE_HOME/.codebuddy/skills/memory-for-ai/SKILL.md"
CODEBUDDY_AGENT="$FAKE_HOME/.codebuddy/agents/memory-for-ai.md"
...
BOB_RULE="$FAKE_HOME/.bob/rules/memory-for-ai.md"
BOB_SKILL="$FAKE_HOME/.bob/skills/memory-for-ai/SKILL.md"
BOB_AGENT="$FAKE_HOME/.bob/agents/memory-for-ai.md"
POCHI_MCP="$FAKE_HOME/.pochi/config.jsonc"
POCHI_INSTRUCTIONS="$FAKE_HOME/.pochi/README.pochi.md"
POCHI_SKILL="$FAKE_HOME/.pochi/skills/memory-for-ai/SKILL.md"
POCHI_AGENT="$FAKE_HOME/.pochi/agents/memory-for-ai.md"
ROVO_MCP="$FAKE_HOME/.rovodev/mcp.json"
ROVO_INSTRUCTIONS="$FAKE_HOME/.rovodev/AGENTS.md"
ROVO_SKILL="$FAKE_HOME/.rovodev/skills/memory-for-ai/SKILL.md"
ROVO_AGENT="$FAKE_HOME/.rovodev/subagents/memory-for-ai.md"
...
if [[ "$BINARY" == *.exe ]]; then
  SELF_PATH="$FAKE_HOME/.local/bin/memory-for-ai.exe"
else
  cp "$BINARY" "$FAKE_HOME/.local/bin/memory-for-ai"
  SELF_PATH="$FAKE_HOME/.local/bin/memory-for-ai"
fi
create_agent_stub() {
  local name="$1"
  if [[ "$BINARY" == *.exe ]]; then
...
if [[ "$BINARY" == *.exe ]]; then
  ...
  PHASE8_CANONICAL="$FAKE_HOME/.local/bin/memory-for-ai.exe"
  ...
  if [ -e "$FAKE_HOME/.local/bin/memory-for-ai.payload.exe" ]; then
...
assert_tier_profile_set() {
  ...
  for spec in \
    "memory-for-ai-scout|Tier 1" \
    "memory-for-ai|Tier 2" \
    "memory-for-ai-auditor|Tier 3"; do
    slug=${spec%%|*}
    tier=${spec##*|}
    ...
    if [ "$access" = "direct" ]; then
      if ! grep -Fq 'source read/grep fallback' "$file" 2>/dev/null; then
        ...
        exit 1
         ...
         grep -qE '(mcpServers|mcp__memory-for-ai__|mcp_memory-for-ai_|@memory-for-ai/|memory-for-ai/)' "$file" 2>/dev/null; then
...
assert_tier_profile_set_removed() {
  ...
  local suffix="$3"
  local slug file
  for slug in memory-for-ai-scout memory-for-ai memory-for-ai-auditor; do
    file="$directory/$slug$suffix"
    if [ -e "$file" ]; then
...
CMD=$(json_get "$FAKE_HOME/.claude.json" "d.get('mcpServers',{}).get('memory-for-ai',{}).get('command','')")
...
sys.exit(0 if 'memory-for-ai' in d.get('mcpServers', {}) else 1)
" 2>/dev/null; then
...
CLAUDE_AGENT="$FAKE_HOME/.claude/agents/memory-for-ai.md"
if ! grep -q '^mcpServers: \[memory-for-ai\]$' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__memory-for-ai__search_graph' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__memory-for-ai__check_index_coverage' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q '^permissionMode: plan$' "$CLAUDE_AGENT" 2>/dev/null ||
   grep -qE 'mcp__memory-for-ai__(index_repository|delete_project|manage_adr|ingest_traces)' "$CLAUDE_AGENT" 2>/dev/null; then
...
# 8f-8h: Codex TOML
if ! grep -q '\[mcp_servers.memory-for-ai\]' "$FAKE_HOME/.codex/config.toml"; then
...
if [ ! -f "$FAKE_HOME/.codex/AGENTS.md" ] || ! grep -q 'memory-for-ai' "$FAKE_HOME/.codex/AGENTS.md"; then
...
CMD=$(json_get "$FAKE_HOME/.gemini/settings.json" "d['mcpServers']['memory-for-ai']['command']")
...
GEMINI_AGENT="$FAKE_HOME/.gemini/agents/memory-for-ai.md"
if ! grep -q '^name: memory-for-ai$' "$GEMINI_AGENT" 2>/dev/null ||
   ...
   ! grep -q 'mcp_memory-for-ai_search_graph' "$GEMINI_AGENT" 2>/dev/null ||
   ! grep -q 'mcp_memory-for-ai_check_index_coverage' "$GEMINI_AGENT" 2>/dev/null ||
   grep -qE 'mcp_memory-for-ai_(index_repository|delete_project|manage_adr|ingest_traces)' "$GEMINI_AGENT" 2>/dev/null; then
...
if [ -f "$ZED_CFG" ]; then
  CMD=$(json_get "$ZED_CFG" "d['context_servers']['memory-for-ai']['command']")
  if ! path_match "$CMD" "$SELF_PATH"; then
...
if ! grep -q 'Memory for AI' "$ZED_INSTR" 2>/dev/null ||
...
CMD=$(json_get "$FAKE_HOME/.config/opencode/opencode.json" "d['mcp']['memory-for-ai']['command'][0]")
if [ -n "$CMD" ]; then
  if ! path_match "$CMD" "$SELF_PATH"; then
...
CMD=$(json_get "$FAKE_HOME/.gemini/config/mcp_config.json" "d['mcpServers']['memory-for-ai']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
if [ -f "$FAKE_HOME/CONVENTIONS.md" ]; then
  if ! grep -q 'memory-for-ai' "$FAKE_HOME/CONVENTIONS.md"; then
...
CMD=$(json_get "$KILO_CFG" "d['mcp']['memory-for-ai']['command'][0]")
if ! path_match "$CMD" "$SELF_PATH"; then
...
KILO_RULE="$FAKE_HOME/.config/kilo/rules/memory-for-ai.md"
if [ ! -f "$KILO_RULE" ]; then
...
KILO_AGENT="$FAKE_HOME/.config/kilo/agents/memory-for-ai.md"
if ! grep -q '^mode: subagent$' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"*": deny' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"memory-for-ai_search_graph": allow' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"memory-for-ai_get_code_snippet": allow' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"memory-for-ai_check_index_coverage": allow' "$KILO_AGENT" 2>/dev/null ||
   grep -Fq '"memory-for-ai_*": allow' "$KILO_AGENT" 2>/dev/null ||
   grep -qE 'memory-for-ai_(index_repository|delete_project|manage_adr|ingest_traces)' "$KILO_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$VSCODE_CFG" "d['servers']['memory-for-ai']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8v: VS Code command='$CMD'"
  exit 1
fi
echo "OK 8v: VS Code MCP"
CMD=$(json_get "$VSCODE_PROFILE_CFG" "d['servers']['memory-for-ai']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
# 8w: OpenClaw MCP
CMD=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "d['mcp']['servers']['memory-for-ai']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8w: OpenClaw command='$CMD'"
  exit 1
fi
ENABLED=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "d['mcp']['servers']['memory-for-ai'].get('enabled')")
...
for OPENCLAW_CONTEXT in \
  ...
  if ! grep -q '^## Codebase Knowledge Graph (memory-for-ai)$' "$OPENCLAW_CONTEXT" 2>/dev/null ||
     ! grep -q 'subagent' "$OPENCLAW_CONTEXT" 2>/dev/null; then
    echo "FAIL 8w-i: OpenClaw durable context missing in $OPENCLAW_CONTEXT"
    exit 1
  fi
done
OPENCLAW_COMPACTION=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "str('Codebase Knowledge Graph (memory-for-ai)' in d['agents']['defaults']['compaction']['postCompactionSections'])")
...
CMD=$(json_get "$FAKE_HOME/.kiro/settings/mcp.json" "d['mcpServers']['memory-for-ai']['command']")
KIRO_AGENT="$FAKE_HOME/.kiro/agents/memory-for-ai.json"
KIRO_AGENT_CMD=$(json_get "$KIRO_AGENT" "d['mcpServers']['memory-for-ai']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! path_match "$KIRO_AGENT_CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$FAKE_HOME/.kiro/steering/memory-for-ai.md" 2>/dev/null ||
   ! cat "$KIRO_AGENT" 2>/dev/null | python3 -c "
import json, sys
d = json.load(sys.stdin)
tools = d.get('tools', [])
server = d.get('mcpServers', {}).get('memory-for-ai', {})
ok = (d.get('name') == 'memory-for-ai' and tools[:3] == ['read', 'grep', 'glob'] and
      '@memory-for-ai/search_graph' in tools and
      '@memory-for-ai/check_index_coverage' in tools and
      all('@memory-for-ai/' + name not in tools for name in
          ('index_repository', 'delete_project', 'manage_adr', 'ingest_traces')) and
      '@memory-for-ai' not in tools and d.get('includeMcpJson') is False and
      set(d.get('mcpServers', {})) == {'memory-for-ai'} and
...
if ! grep -q '^name: memory-for-ai$' "$FAKE_HOME/.hermes/skills/memory-for-ai/SKILL.md" 2>/dev/null ||
   ! grep -q 'delegate_task' "$FAKE_HOME/.hermes/skills/memory-for-ai/SKILL.md" 2>/dev/null ||
   ! grep -q '`context`' "$FAKE_HOME/.hermes/skills/memory-for-ai/SKILL.md" 2>/dev/null; then
...
# 8y: OpenHands MCP
CMD=$(json_get "$FAKE_HOME/.openhands/mcp.json" "d['mcpServers']['memory-for-ai']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8y: OpenHands command='$CMD'"
  exit 1
fi
echo "OK 8y: OpenHands MCP"
if ! grep -q '^name: memory-for-ai$' "$FAKE_HOME/.agents/skills/memory-for-ai/SKILL.md" 2>/dev/null ||
   ! grep -q 'trace_path' "$FAKE_HOME/.agents/skills/memory-for-ai/SKILL.md" 2>/dev/null; then
...
CLINE_RULE="$FAKE_HOME/.cline/rules/memory-for-ai.md"
for CLINE_CFG in "$FAKE_HOME/.cline/mcp.json" \
                 "$FAKE_HOME/.cline/data/settings/cline_mcp_settings.json"; do
  CMD=$(json_get "$CLINE_CFG" "d['mcpServers']['memory-for-ai']['command']")
  if ! path_match "$CMD" "$SELF_PATH"; then
...
CMD=$(json_get "$FAKE_HOME/.qwen/settings.json" "d['mcpServers']['memory-for-ai']['command']")
...
COPILOT_SKILL="$FAKE_HOME/.copilot/skills/memory-for-ai/SKILL.md"
COPILOT_AGENT="$FAKE_HOME/.copilot/agents/memory-for-ai.agent.md"
if ! grep -q 'search_graph' "$COPILOT_SKILL" 2>/dev/null ||
   ! grep -q '^tools:' "$COPILOT_AGENT" 2>/dev/null ||
   ! grep -q 'memory-for-ai/trace_path' "$COPILOT_AGENT" 2>/dev/null ||
   grep -qE '^  - (edit|shell|bash|memory-for-ai/(index_repository|delete_project|manage_adr|ingest_traces))$' "$COPILOT_AGENT" 2>/dev/null; then
...
COPILOT_HOOKS="$FAKE_HOME/.copilot/hooks/memory-for-ai.json"
...
CMD=$(json_get "$FAKE_HOME/.factory/mcp.json" "d['mcpServers']['memory-for-ai']['command']")
FACTORY_TYPE=$(json_get "$FAKE_HOME/.factory/mcp.json" "d['mcpServers']['memory-for-ai']['type']")
...
FACTORY_AGENT="$FAKE_HOME/.factory/droids/memory-for-ai.md"
if ! grep -q '^tools: \["Read", "LS", "Grep", "Glob",' "$FACTORY_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__memory-for-ai__search_graph' "$FACTORY_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__memory-for-ai__check_index_coverage' "$FACTORY_AGENT" 2>/dev/null ||
   grep -q '^mcpServers:' "$FACTORY_AGENT" 2>/dev/null ||
   grep -qE 'mcp__memory-for-ai__(index_repository|delete_project|manage_adr|ingest_traces)' "$FACTORY_AGENT" 2>/dev/null; then
...
CMD=$(json_get "$FAKE_HOME/.config/crush/crush.json" "d['mcp']['memory-for-ai']['command']")
CRUSH_TYPE=$(json_get "$FAKE_HOME/.config/crush/crush.json" "d['mcp']['memory-for-ai']['type']")
CRUSH_CONTEXT=$(json_get "$FAKE_HOME/.config/crush/crush.json" "str(any(str(p).endswith('memory-for-ai.md') for p in d['options']['context_paths']))")
if ! path_match "$CMD" "$SELF_PATH" || [ "$CRUSH_TYPE" != "stdio" ] ||
   [ "$CRUSH_CONTEXT" != "True" ] ||
   ! grep -q 'does not inherit MCP access' "$FAKE_HOME/.config/crush/memory-for-ai.md" 2>/dev/null; then
...
GOOSE_CMD=$(sed -n '/^  memory-for-ai:/,/^  [^ ]/{s/^ *cmd: *//p;}' \
  "$GOOSE_CFG" 2>/dev/null | head -1)
if ! grep -q '^extensions:' "$GOOSE_CFG" 2>/dev/null ||
   ! grep -q '^  memory-for-ai:' "$GOOSE_CFG" 2>/dev/null ||
...
if ! grep -q '^\[\[mcp_servers\]\]' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null ||
   ! grep -q '^name = "memory-for-ai"' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null ||
...
VIBE_AGENT="$FAKE_HOME/.vibe/agents/memory-for-ai.toml"
VIBE_PROMPT="$FAKE_HOME/.vibe/prompts/memory-for-ai.md"
if ! grep -q '^agent_type = "subagent"$' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq 'enabled_tools = ["read_file", "grep_search", "memory-for-ai_search_graph"' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq '"memory-for-ai_get_code_snippet"' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq '"memory-for-ai_check_index_coverage"' "$VIBE_AGENT" 2>/dev/null ||
   grep -Fq '"memory-for-ai_*"' "$VIBE_AGENT" 2>/dev/null ||
   grep -qE 'memory-for-ai_(index_repository|delete_project|manage_adr|ingest_traces)' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -q '^system_prompt_id = "memory-for-ai"$' "$VIBE_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$FAKE_HOME/.codeium/windsurf/mcp_config.json" "d['mcpServers']['memory-for-ai']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
...
AUGMENT_RULE="$FAKE_HOME/.augment/rules/memory-for-ai.md"
AUGMENT_AGENT="$FAKE_HOME/.augment/agents/memory-for-ai.md"
if [[ "$BINARY" == *.exe ]]; then
  AUGMENT_SCRIPT="$FAKE_HOME/.augment/hooks/memory-for-ai-session.ps1"
else
  AUGMENT_SCRIPT="$FAKE_HOME/.augment/hooks/memory-for-ai-session.sh"
fi
CMD=$(json_get "$AUGMENT_SETTINGS" "d['mcpServers']['memory-for-ai']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$AUGMENT_RULE" 2>/dev/null ||
   ! grep -q '^name: memory-for-ai$' "$AUGMENT_AGENT" 2>/dev/null ||
...
owned = [e for e in entries if any('memory-for-ai-session' in str(h.get('command', '')) for h in e.get('hooks', []))]
...
SKILL_FILE="$FAKE_HOME/.claude/skills/memory-for-ai/SKILL.md"
if [ ! -s "$SKILL_FILE" ]; then
  echo "FAIL 8ai: skill memory-for-ai missing or empty"
  exit 1
fi
...
QODER_SKILL="$FAKE_HOME/.qoder/skills/memory-for-ai/SKILL.md"
QODER_AGENT="$FAKE_HOME/.qoder/agents/memory-for-ai.md"
CMD=$(json_get "$QODER_SETTINGS" "d['mcpServers']['memory-for-ai']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$QODER_SKILL" 2>/dev/null ||
   ! grep -q '^tools: Read,Grep,Glob,mcp__memory-for-ai__search_graph' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__memory-for-ai__check_index_coverage' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q '^mcpServers:$' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q '^  - memory-for-ai$' "$QODER_AGENT" 2>/dev/null ||
   grep -qE 'mcp__memory-for-ai__(index_repository|delete_project|manage_adr|ingest_traces)' "$QODER_AGENT" 2>/dev/null ||
...
KIMI_SKILL="$CUSTOM_KIMI_HOME/skills/memory-for-ai/SKILL.md"
KIMI_CONFIG="$CUSTOM_KIMI_HOME/config.toml"
CMD=$(json_get "$KIMI_MCP" "d['mcpServers']['memory-for-ai']['command']")
...
PI_SKILL="$FAKE_HOME/.pi/agent/skills/memory-for-ai/SKILL.md"
...
WARP_SKILL="$FAKE_HOME/.agents/skills/memory-for-ai/SKILL.md"
...
JUNIE_SKILL="$FAKE_HOME/.junie/skills/memory-for-ai/SKILL.md"
JUNIE_AGENT="$FAKE_HOME/.junie/agents/memory-for-ai.md"
CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['memory-for-ai']['command']")
JUNIE_SCOUT_CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['memory-for-ai-scout']['command']")
JUNIE_ANALYSIS_CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['memory-for-ai-analysis']['command']")
JUNIE_SCOUT_ARGS=$(json_get "$JUNIE_MCP" "d['mcpServers']['memory-for-ai-scout']['args']")
JUNIE_ANALYSIS_ARGS=$(json_get "$JUNIE_MCP" "d['mcpServers']['memory-for-ai-analysis']['args']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ...
   ! grep -q 'mcpServers: \["memory-for-ai-analysis"\]' "$JUNIE_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$ROO_CFG" "d['mcpServers']['memory-for-ai']['command']")
...
CMD=$(json_get "$GITLAB_MCP" "d['mcpServers']['memory-for-ai']['command']")
...
CMD=$(json_get "$DEVIN_CONFIG" "d['mcpServers']['memory-for-ai']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
...
CMD=$(json_get "$CODEBUDDY_MCP" "d['mcpServers']['memory-for-ai']['command']")
...
if ! path_match "$CMD" "$SELF_PATH" || [ "$CODEBUDDY_KEEP" != "codebuddy" ] ||
   ...
   ! grep -q '^tools: Read,Grep,Glob,mcp__memory-for-ai__search_graph,' "$CODEBUDDY_AGENT" 2>/dev/null ||
...
BOB_IDE_CMD=$(json_get "$BOB_IDE_MCP" "d['mcpServers']['memory-for-ai']['command']")
BOB_SHELL_CMD=$(json_get "$BOB_SHELL_MCP" "d['mcpServers']['memory-for-ai']['command']")
...
POCHI_CMD=$(sed '/^[[:space:]]*\/\//d' "$POCHI_MCP" 2>/dev/null | python3 -c "import json,sys; print(json.load(sys.stdin)['mcp']['memory-for-ai']['command'])" 2>/dev/null || echo "")
...
ROVO_CMD=$(json_get "$ROVO_MCP" "d['mcpServers']['memory-for-ai']['command']")
...
AMAZON_Q_CMD=$(json_get "$AMAZON_Q_MCP" "d['mcpServers']['memory-for-ai']['command']")
...
for VIBE_SLUG in memory-for-ai-scout memory-for-ai memory-for-ai-auditor; do
...
if ! grep -q '^\[mcp_servers\.memory-for-ai\]$' "$GROK_CONFIG" 2>/dev/null ||
   [ "$(grep -c '^\[mcp_servers\.memory-for-ai\]$' "$GROK_CONFIG" 2>/dev/null)" != "1" ] ||
...
if ! grep -q 'search_graph' "$FAKE_HOME/.grok/rules/memory-for-ai.md" 2>/dev/null ||
   ! grep -q 'search_graph' "$FAKE_HOME/.grok/skills/memory-for-ai/SKILL.md" 2>/dev/null; then
  echo "FAIL 8ax: Grok Build rules file or skill missing"
  exit 1
fi
GROK_AGENT="$FAKE_HOME/.grok/agents/memory-for-ai.md"
if ! grep -Fq 'tools: read_file, grep, list_dir, search_tool, use_tool' "$GROK_AGENT" 2>/dev/null ||
   ! grep -Fq '    - memory-for-ai' "$GROK_AGENT" 2>/dev/null ||
   ! grep -Fq 'memory-for-ai__search_graph' "$GROK_AGENT" 2>/dev/null ||
   ! grep -Fq 'memory-for-ai__check_index_coverage' "$GROK_AGENT" 2>/dev/null ||
   grep -Fq 'memory-for-ai__*' "$GROK_AGENT" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
if 'memory-for-ai' in d.get('mcpServers', {}):
    sys.exit(1)
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'memory-for-ai' in d.get('mcpServers', {}) else 0)
" 2>/dev/null; then
...
if grep -q '\[mcp_servers.memory-for-ai\]' "$FAKE_HOME/.codex/config.toml" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
has_mcp = 'memory-for-ai' in d.get('mcpServers', {})
has_existing = d.get('existingKey', False)
hooks = d.get('hooks', {}).get('BeforeTool', [])
has_hook = any('memory-for-ai' in str(h) for h in hooks)
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'memory-for-ai' in d.get('servers', {}) else 0)
" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
has_mcp = 'memory-for-ai' in d.get('mcp', {}).get('servers', {})
sections = d.get('agents', {}).get('defaults', {}).get('compaction', {}).get('postCompactionSections', [])
sys.exit(1 if has_mcp or 'Codebase Knowledge Graph (memory-for-ai)' in sections else 0)
" 2>/dev/null; then
  if grep -q 'memory-for-ai:start' "$FAKE_HOME/.openclaw/workspace/AGENTS.md" 2>/dev/null ||
     grep -q 'memory-for-ai:start' "$FAKE_HOME/.openclaw/workspace/TOOLS.md" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'memory-for-ai' in d.get('$ROOT', {}) else 0)
" 2>/dev/null; then
...
names = {'memory-for-ai', 'memory-for-ai-scout', 'memory-for-ai-analysis'}
...
import json, sys
d = json.load(sys.stdin)
print('present' if 'memory-for-ai' in d.get('mcp', {}) else 'absent')
...
if grep -q 'hook-augment' "$QODER_SETTINGS" 2>/dev/null ||
   ...
   grep -q 'hook-augment' "$FAKE_HOME/.copilot/hooks/memory-for-ai.json" 2>/dev/null ||
   grep -q 'hook-augment' "$FAKE_HOME/.factory/hooks.json" 2>/dev/null ||
   grep -q 'memory-for-ai-session' "$AUGMENT_SETTINGS" 2>/dev/null ||
...
if CRUSH_CONTEXT=$(json_get "$FAKE_HOME/.config/crush/crush.json" "str(any(str(p).endswith('memory-for-ai.md') for p in d.get('options', {}).get('context_paths', [])))") &&
...
if grep -q '^  memory-for-ai:' "$FAKE_HOME/.hermes/config.yaml" 2>/dev/null ||
   grep -q '^  pre_llm_call:' "$FAKE_HOME/.hermes/config.yaml" 2>/dev/null ||
   grep -q '^  memory-for-ai:' "$GOOSE_CFG" 2>/dev/null ||
   grep -q 'memory-for-ai' "$FAKE_HOME/.grok/config.toml" 2>/dev/null ||
   grep -q '^name = "memory-for-ai"' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null; then
...
for CONTEXT_FILE in \
  "$ZED_INSTR" \
  "$FAKE_HOME/.kiro/steering/memory-for-ai.md" \
  "$FAKE_HOME/.factory/AGENTS.md" \
  "$AUGMENT_RULE" \
  "$FAKE_HOME/.config/crush/memory-for-ai.md" \
  "$FAKE_HOME/.config/goose/.goosehints" \
  "$KILO_RULE" \
  "$CLINE_RULE" \
  "$FAKE_HOME/.vibe/AGENTS.md" \
  "$FAKE_HOME/.grok/rules/memory-for-ai.md" \
  ...
  "$PI_INSTRUCTIONS"; do
  if grep -q 'memory-for-ai:start' "$CONTEXT_FILE" 2>/dev/null; then
...
if [ -d "$FAKE_HOME/.claude/skills/memory-for-ai" ] ||
   [ -d "$FAKE_HOME/.hermes/skills/memory-for-ai" ] ||
   [ -d "$FAKE_HOME/.agents/skills/memory-for-ai" ] ||
   [ -d "$FAKE_HOME/.qoder/skills/memory-for-ai" ] ||
   [ -d "$CUSTOM_KIMI_HOME/skills/memory-for-ai" ] ||
   [ -d "$FAKE_HOME/.pi/agent/skills/memory-for-ai" ] ||
   [ -d "$FAKE_HOME/.junie/skills/memory-for-ai" ] ||
   [ -d "$FAKE_HOME/.rovodev/skills/memory-for-ai" ] ||
   [ -d "$FAKE_HOME/.copilot/skills/memory-for-ai" ] ||
   [ -d "$FAKE_HOME/.vibe/skills/memory-for-ai" ] ||
   [ -d "$FAKE_HOME/.grok/skills/memory-for-ai" ] ||
   [ -e "$DEVIN_SKILL" ] ||
   [ -e "$CODEBUDDY_SKILL" ] ||
   [ -e "$BOB_SKILL" ] ||
   [ -e "$POCHI_SKILL" ] ||
...
import json, sys
d = json.load(sys.stdin)
print(list(d.get('mcpServers',{}).keys()).count('memory-for-ai'))
" 2>/dev/null || echo "0")
if [ "$COUNT" != "1" ]; then
...
copy_smoke_binary "$CORRUPT_HOME/.local/bin/memory-for-ai"
...
copy_smoke_binary "$DBL_HOME/.local/bin/memory-for-ai"
run_no_crash 9b-8-install env HOME="$DBL_HOME" "$BINARY" install -y
DBL_UNINSTALLER="$BINARY"
if [[ "$BINARY" == *.exe ]]; then
  DBL_UNINSTALLER="$DBL_HOME/.local/bin/memory-for-ai.exe"
fi
...
SECURITY_BIN="$SECURITY_DIR/memory-for-ai"
...
if [ -n "${SMOKE_DOWNLOAD_URL:-}" ]; then
  ...
  if [[ "$BINARY" == *.exe ]]; then
    cp "$BINARY" "$UPDATE_HOME/.local/bin/memory-for-ai.exe"
    mkdir -p "$UPDATE_HOME/retired-install"
    cp "$BINARY" "$UPDATE_HOME/retired-install/memory-for-ai.exe"
  else
    cp "$BINARY" "$UPDATE_HOME/.local/bin/memory-for-ai"
    chmod 755 "$UPDATE_HOME/.local/bin/memory-for-ai"
    mkdir -p "$UPDATE_HOME/retired-install"
    cp "$BINARY" "$UPDATE_HOME/retired-install/memory-for-ai"
    chmod 755 "$UPDATE_HOME/retired-install/memory-for-ai"
    if [ "$(uname -s)" = "Darwin" ]; then
      codesign --sign - --force "$UPDATE_HOME/.local/bin/memory-for-ai" 2>/dev/null || true
      codesign --sign - --force "$UPDATE_HOME/retired-install/memory-for-ai" \
        2>/dev/null || true
    fi
  fi
  ...
  # uninstall phases.
  if [[ "$BINARY" == *.exe ]]; then
    UPDATE_DRIVER="$UPDATE_HOME/.local/bin/memory-for-ai.exe"
  else
    UPDATE_DRIVER="$UPDATE_HOME/.local/bin/memory-for-ai"
  fi
  ...
  STALE_CMD="$STALE_CMD" python3 -c \
    'import json, os; print(json.dumps({"mcpServers":{"memory-for-ai":{"command":os.environ["STALE_CMD"]}}}))' \
  ...
  if [[ "$BINARY" == *.exe ]]; then
    UPD_BIN="$UPDATE_HOME/.local/bin/memory-for-ai.exe"
  else
    UPD_BIN="$UPDATE_HOME/.local/bin/memory-for-ai"
  fi
  if [ ! -f "$UPD_BIN" ]; then
  ...
  if [ -f "$UPDATE_HOME/.local/bin/memory-for-ai" ] || [ -f "$UPDATE_HOME/.local/bin/memory-for-ai.exe" ]; then
...
import json, sys
d = json.load(sys.stdin)
if 'memory-for-ai' in d.get('mcpServers', {}): sys.exit(1)
sys.exit(0)
" 2>/dev/null; then
  ...
  copy_smoke_binary "$UPDATE_DIR/install/memory-for-ai"
  chmod 755 "$UPDATE_DIR/install/memory-for-ai"
  copy_smoke_binary "$UPDATE_DIR/smoke-downloaded"
  rm -f "$UPDATE_DIR/install/memory-for-ai"
  cp "$UPDATE_DIR/smoke-downloaded" "$UPDATE_DIR/install/memory-for-ai"
  chmod 755 "$UPDATE_DIR/install/memory-for-ai"
  if [ "$(uname -s)" = "Darwin" ]; then
    codesign --sign - --force "$UPDATE_DIR/install/memory-for-ai" 2>/dev/null || true
  fi
  if ! "$UPDATE_DIR/install/memory-for-ai" --version > /dev/null 2>&1; then
...
DL_ARCHIVE="memory-for-ai-${DL_OS}-${DL_ARCH}.${DL_EXT}"
...
if [ "$DL_OS" != "windows" ] && [ -f "$REPO_ROOT/install.sh" ]; then
  ...
  # 13b: binary placed
  if [ ! -f "$INSTALL_TEST_DIR/memory-for-ai" ]; then
  ...
  if [ "$DL_OS" = "darwin" ]; then
    codesign --sign - --force "$INSTALL_TEST_DIR/memory-for-ai" 2>/dev/null || true
  fi
  if ! "$INSTALL_TEST_DIR/memory-for-ai" --version > /dev/null 2>&1; then
  ...
  if [ "$DL_OS" = "darwin" ]; then
    if codesign -v "$INSTALL_TEST_DIR/memory-for-ai" 2>/dev/null; then
  ...
  if [ -f "$INSTALL_TEST_HOME/.claude.json" ] && grep -q 'memory-for-ai' "$INSTALL_TEST_HOME/.claude.json" 2>/dev/null; then
  ...
  # 13g: binary placed
  PS1_BIN="$PS1_TEST_DIR/memory-for-ai.exe"
  if [ ! -f "$PS1_BIN" ] && [ -f "$PS1_TEST_DIR/memory-for-ai" ]; then
    PS1_BIN="$PS1_TEST_DIR/memory-for-ai"
  fi
  if [ -f "$PS1_BIN" ]; then
