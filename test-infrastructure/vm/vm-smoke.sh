...
Environment:
  ...
  CBM_SMOKE_ARTIFACT_DIR
                  ...
                  (memory-for-ai.exe + LICENSE + install.ps1 +
...
if [ -n "$ARTIFACT_DIR" ]; then
    ...
    for required in memory-for-ai.exe \
    ...
    BINARY_SRC="$ARTIFACT_DIR/memory-for-ai.exe"
else
    BINARY_SRC="build/c/memory-for-ai.exe"
...
cp "$SMOKE_DIR/memory-for-ai.exe" "$FIXTURE_DIR/"
...
EXPECTED_ARTIFACT="memory-for-ai-windows-${SMOKE_ARCH}.zip"
(
    cd "$FIXTURE_DIR"
    zip -q "$EXPECTED_ARTIFACT" \
        memory-for-ai.exe LICENSE install.ps1 THIRD_PARTY_NOTICES.md
...
env \
    ...
    scripts/smoke-test.sh "$SMOKE_DIR/memory-for-ai.exe"
