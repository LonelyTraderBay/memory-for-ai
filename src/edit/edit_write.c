...
static int edit_backup_directory(const char *root, const char *path, char *out, size_t cap) {
    ...
    if (base == path || !*base || strcmp(base, ".") == 0 || strcmp(base, "..") == 0)
        return CBM_EDIT_ERR_ARGS;
    char parent[CBM_SZ_4K], canonical[CBM_SZ_4K], identity[CBM_SZ_4K];
...
static uint64_t edit_backup_sequence(const char *name) {
    if (strlen(name) != 23 || strncmp(name, "bk_", 3) != 0)
        return 0;
    for (int i = 3; i < 23; i++)
        if (name[i] < '0' || name[i] > '9')
            return 0;
    errno = 0;
    unsigned long long value = strtoull(name + 3, NULL, 10);
...
int cbm_edit_write_atomic(const char *abs_path, const char *data, size_t len,
    ...
    if (backup_dir) {
        char dir[CBM_SZ_4K], backup[CBM_SZ_4K], tmp[CBM_SZ_4K];
        rc = edit_backup_directory(backup_dir, abs_path, dir, sizeof(dir));
        if (rc != CBM_EDIT_OK)
            return rc;
        if (!cbm_mkdir_p(dir, 0700))
            return CBM_EDIT_ERR_IO;
        uint64_t last;
        rc = edit_backup_last_sequence(dir, &last);
        if (rc != CBM_EDIT_OK || last == UINT64_MAX)
            return CBM_EDIT_ERR_IO;
        int n = snprintf(backup, sizeof(backup), "%s/bk_%020" PRIu64, dir, last + 1);
        if (n < 0 || (size_t)n >= sizeof(backup) ||
            (backup_path_out && (size_t)n >= backup_path_sz))
            return CBM_EDIT_ERR_ARGS;
        ...
        if (backup_path_out)
            memcpy(backup_path_out, backup, strlen(backup) + 1);
    }
