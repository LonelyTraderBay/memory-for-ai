 ...
 * to .memory-for-ai/graph.db.zst in the repository. Teammates
...
#define CBM_ARTIFACT_DIR ".memory-for-ai"
...
/* Export DB to .memory-for-ai/graph.db.zst artifact.
 * quality: CBM_ARTIFACT_FAST or CBM_ARTIFACT_BEST.
 * Creates .memory-for-ai/ dir, .gitattributes, and artifact.json.
...
/* Import artifact from .memory-for-ai/graph.db.zst to cache_db_path.
 * Decompresses, runs integrity check, recreates indexes.
 * Returns 0 on success, -1 on error. */
int cbm_artifact_import(const char *repo_path, const char *cache_db_path);

/* Check if a compatible artifact exists in repo_path/.memory-for-ai/.
