...
/* Build path: <repo>/.memory-for-ai/<name> into caller-owned buf. */
 ...
 * .memory-for-ai/. The export itself writes .memory-for-ai/, so a blanket
...
static bool tree_clean_for_reconcile(const char *repo_path) {
    /* Tracked (staged + unstaged) changes vs HEAD, excluding .memory-for-ai. */
    if (!git_run_ok(repo_path, "diff --quiet HEAD -- . \":(exclude).memory-for-ai\"")) {
        return false;
    }
    /* Untracked (non-ignored) files outside .memory-for-ai. */
    static const char *const ls_args =
        "ls-files -z --others --exclude-standard -- . \":(exclude).memory-for-ai\"";
    char *untracked = NULL;
    size_t un_len = 0;
 ...
 * synthetic "semantic input" digests under .memory-for-ai/.semantic-input/
 ...
 *
 * Testing the whole .memory-for-ai/ subtree (rather than the semantic-input
 * prefix alone) keeps this in lockstep with the ":(exclude).memory-for-ai"
...
static void ensure_gitattributes(const char *repo_path) {
    ...
    if (fd < 0) {
        ...
        if (fp) {
            ...
            (void)fputs("# Auto-generated by memory-for-ai\n"
...
int cbm_artifact_export(const char *db_path, const char *repo_path, const char *project_name,
    ...
    /* Ensure .memory-for-ai/ directory exists */
