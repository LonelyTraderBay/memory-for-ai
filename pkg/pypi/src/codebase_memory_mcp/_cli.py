"""Downloads memory-for-ai on first run, then runs its native entry point."""
...
_WINDOWS_BINARY_NAME = "memory-for-ai.exe"
_UNIX_ARCHIVE_NAMES = (
    "memory-for-ai",
    "LICENSE",
    "install.sh",
...
_RUNTIME_LOCK_NAME = ".memory-for-ai-runtime.lock"
...
def _validate_url_scheme(url: str) -> None:
    ...
    if (
        ...
        sys.exit(
            f"memory-for-ai: refusing to fetch invalid, credentialed, or "
            f"non-https URL: {url}"
        )
...
def _download_https(url: str, dest: str, max_bytes: int = 0) -> None:
    ...
    for redirect_count in range(_MAX_REDIRECTS + 1):
        ...
        request = urllib.request.Request(
            current_url, headers={"User-Agent": "memory-for-ai-installer"}
        )
        try:
...
def _validate_archive_names(names, archive_names, casefold: bool = False):
    ...
    for name in names:
        ...
        if key in seen:
            sys.exit(
                f"memory-for-ai: refusing unsafe archive entry "
                f"(duplicate or case conflict: {name!r})"
            )
        seen.add(key)
        if name not in required_set:
            sys.exit(
                f"memory-for-ai: archive must contain only the exact "
                f"root files: {', '.join(required)}"
            )
        found.add(name)
    if found != required_set or len(seen) != len(required):
        sys.exit(
            f"memory-for-ai: archive must contain exactly one of each "
...
def _safe_extract_tar(
    ...
    for member in members:
        if not member.isfile():
            sys.exit(
                f"memory-for-ai: refusing unsafe tar entry "
    ...
    for name in runtime_names:
        ...
        if source is None:
            sys.exit(f"memory-for-ai: could not read archive member: {name}")
...
def _safe_extract_zip(
    ...
    for info in zf.infolist():
        ...
        if (
            ...
            sys.exit(
                f"memory-for-ai: refusing unsafe zip entry "
...
def _verify_checksum(archive_path: str, archive_name: str, version: str) -> None:
    ...
    try:
        ...
        with open(tmp_path, encoding="utf-8") as f:
            for line in f:
                ...
                if len(digest) != 64 or any(
                    ...
                    sys.exit(
                        f"memory-for-ai: invalid SHA256 checksum for "
                        f"{archive_name}"
                    )
                if expected is not None and expected != digest:
                    sys.exit(
                        f"memory-for-ai: conflicting SHA256 checksums for "
                        f"{archive_name}"
                    )
                expected = digest
        if expected is None:
            sys.exit(
                f"memory-for-ai: no checksum for {archive_name} in checksums.txt"
            )
        h = hashlib.sha256()
        ...
        if expected != actual:
            sys.exit(
                f"memory-for-ai: CHECKSUM MISMATCH for {archive_name}\n"
                f"  expected: {expected}\n"
                f"  actual:   {actual}"
            )
        print("memory-for-ai: checksum verified.", file=sys.stderr)
    except SystemExit:
        raise
    except Exception as exc:
        sys.exit(f"memory-for-ai: checksum verification failed: {exc}")
    finally:
...
def _download(version: str) -> Path:
    ...
    archive = f"memory-for-ai-{os_name}-{arch}{portable}.{ext}"
    ...
    print(
        f"memory-for-ai: downloading v{version} for {os_name}/{arch}...",
        file=sys.stderr,
    )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            sys.exit(
                f"memory-for-ai: download failed ({e})\n"
                f"URL: {url}\n"
        ...
        bin_name = (
            _WINDOWS_BINARY_NAME if os_name == "windows" else "memory-for-ai"
        )
        ...
        for name in runtime_names:
            ...
            if not _regular_file(extracted_path):
                sys.exit(
                    f"memory-for-ai: required runtime member not found after "
                    f"extraction: {name}"
                )
            if name == bin_name:
        ...
        if _runtime_set_names(Path(tmp), bin_name) is None:
            sys.exit(
                "memory-for-ai: extracted runtime set failed content "
                "verification"
            )
        try:
            _verify_candidate(extracted_paths[bin_name])
        except (OSError, RuntimeError) as exc:
            sys.exit(f"memory-for-ai: {exc}")
    ...
    if mutation in ("install", "uninstall"):
        ...
        if sys.platform == "win32" and result != 0 and mutation == "uninstall":
            print(
                ...
                '"python -m pip uninstall memory-for-ai" for package maintenance. '
...
def _default_managed_install_dir() -> Path:
    ...
    return local_app_data / "Programs" / "memory-for-ai"
