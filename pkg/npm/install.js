...
const REPO = 'LonelyTraderBay/memory-for-ai';
...
const WINDOWS_BINARY_NAME = 'memory-for-ai.exe';
...
const WINDOWS_PAIR_LOCK_NAME = '.memory-for-ai-pair.lock';
...
async function verifyChecksum(archivePath, archiveName) {
  ...
  try {
    ...
    process.stdout.write('memory-for-ai: checksum verified.\n');
  } finally {
...
async function main() {
  ...
  const archive = `memory-for-ai-${platform}-${arch}${variant}.${ext}`;
  const url = `https://github.com/${REPO}/releases/download/v${VERSION}/${archive}`;

  process.stdout.write(`memory-for-ai: downloading v${VERSION} for ${platform}/${arch}...\n`);
  ...
  try {
    ...
    process.stdout.write('memory-for-ai: ready.\n');
    if (platform === 'windows') {
...
if (require.main === module || module.parent == null) {
  main().catch((err) => {
    process.stderr.write(`\nmemory-for-ai: install failed — ${err.message}\n`);
