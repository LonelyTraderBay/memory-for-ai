...
const binName = isWindows ? 'memory-for-ai.exe' : 'memory-for-ai';
...
if (!cacheReady()) {
  ...
  process.stderr.write('memory-for-ai: binary not found, downloading...\n');
  const installResult = spawnSync(process.execPath, [path.join(__dirname, 'install.js')], {
    stdio: 'inherit',
  });
  if (installResult.status !== 0 || !cacheReady()) {
    process.stderr.write(
      'memory-for-ai: download failed.\n' +
      'Try reinstalling: npm install -g memory-for-ai\n'
    );
    process.exit(1);
  }
}
...
if (result.error) {
  process.stderr.write(`memory-for-ai: ${result.error.message}\n`);
  process.exit(1);
}
...
if (mutation) {
  ...
  process.stderr.write(
    ...
    '"memory-for-ai install --yes" once for a managed install with its ' +
