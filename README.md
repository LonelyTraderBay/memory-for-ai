...
[![VirusTotal](https://img.shields.io/badge/VirusTotal-scanned_every_release-brightgreen?logo=virustotal)](https://github.com/LonelyTraderBay/memory-for-ai/releases/latest)
...
1. **Download** the archive for your platform from the [latest release](https://github.com/LonelyTraderBay/memory-for-ai/releases/latest):
   - `memory-for-ai-<os>-<arch>.tar.gz` (macOS/Linux) or `.zip` (Windows)

2. **Extract and install** (each archive includes `install.sh` or `install.ps1`):

   macOS / Linux:
   ```bash
   tar xzf memory-for-ai-*.tar.gz
   ./install.sh
   ```

   Windows (PowerShell):
   ```powershell
   Expand-Archive memory-for-ai-windows-amd64.zip -DestinationPath .
   Unblock-File .\install.ps1
   .\install.ps1
   ```
...
The detached daemon does not depend on an MCP frontend's stderr. It keeps owner-only durable records under the canonical `${MFA_CACHE_DIR}/logs` directory (default `~/.cache/memory-for-ai/logs`):

| File | Contents |
|------|----------|
| `memory-for-ai-daemon.log` | Daemon lifecycle, watcher/indexing, UI, resource, and error events. |
...
All active CBM processes must run the exact same version, executable build, coordination ABI, and canonical cache root. Equivalent `MFA_CACHE_DIR` aliases resolve to the same root; a genuinely different root is rejected while any CBM process is active. MCP servers, hooks, one-shot CLI commands, temporary index workers, and the daemon share a crash-safe OS admission barrier; starting an ordinary conflicting process fails before doing work and records an explicit conflict in `${MFA_CACHE_DIR}/logs/daemon-conflicts.ndjson`.

The native `install`, `update`, and `uninstall` commands are the deliberate exception to that conflict rule. Download, verification, and private same-filesystem staging happen first so a bad candidate never disrupts active work. Activation then publishes account-wide maintenance intent, asks the daemon and every temporary local operation to cancel, and waits to a finite deadline for all coordinated CBM processes to exit. It holds the admission and lifetime barriers exclusively while changing the active binary, configuration, PATH, or indexes. New CBM work cannot enter during this window. Activation progress and results are recorded in `${MFA_CACHE_DIR}/logs/activation-events.ndjson`, and a successful command tells you to restart open coding-agent sessions so they launch the activated build.
...
```bash
memory-for-ai --ui=true --port=9749
```
...
To turn the watcher off entirely, set `config set watcher_enabled false` (default `true`): the background poll thread never starts and no project is registered, while `auto_index` and manual `index_repository` keep working. Unlike `auto_watch` — which is consulted per session — `watcher_enabled` is read once when the background daemon starts, so run `memory-for-ai daemon stop` after changing it; reconnecting your MCP client alone will not restart the daemon. See [docs/CONFIGURATION.md](docs/CONFIGURATION.md#2-cli-managed-runtime-settings).

### Keeping Up to Date

**Updates run from the install script on every platform, not from inside the running binary.** `memory-for-ai update` validates your flags and then prints the exact command to run:
...
Installed through **npm or pip**? Update with your package manager on every platform (`npm install -g memory-for-ai@latest` / `pip install -U memory-for-ai`).
...
`.memory-for-ai/graph.db.zst` is a zstd-compressed snapshot of the knowledge graph that lives next to your source. When you index, the artifact is written or refreshed; when a teammate clones the repo and runs `memory-for-ai` for the first time, the artifact is decompressed and incremental indexing fills in their local diff.
...
- **Optional**: never committed unless you want it. Add `.memory-for-ai/` to `.gitignore` if you prefer everyone to reindex from scratch.
...
memory-for-ai is a **structural analysis backend** — it builds and queries the knowledge graph. It does **not** include an LLM. Instead, it relies on your MCP client (Claude Code, or any MCP-compatible agent) to be the intelligence layer.
...
memory-for-ai: executes graph query, returns structured results
...
**Token efficiency**: Five structural queries consumed ~3,400 tokens via memory-for-ai versus ~412,000 tokens via file-by-file grep exploration — a **99.2% reduction**.
...
| Platform | Archive |
|----------|---------|
| macOS (Apple Silicon) | `memory-for-ai-darwin-arm64.tar.gz` |
| macOS (Intel) | `memory-for-ai-darwin-amd64.tar.gz` |
| Linux (x86_64) | `memory-for-ai-linux-amd64.tar.gz` |
| Linux (ARM64) | `memory-for-ai-linux-arm64.tar.gz` |
| Windows (x86_64) | `memory-for-ai-windows-amd64.zip` |
...
```bash
curl -fsSL https://raw.githubusercontent.com/LonelyTraderBay/memory-for-ai/main/scripts/setup.sh | bash
```

**Windows (PowerShell):**

```powershell
irm https://raw.githubusercontent.com/LonelyTraderBay/memory-for-ai/main/scripts/setup-windows.ps1 | iex
```
...
```bash
yay -S memory-for-ai-bin
```

```bash
paru -S memory-for-ai-bin
```

The `memory-for-ai-bin` package is available at: https://aur.archlinux.org/packages/memory-for-ai-bin
...
| Package | Contents |
|---------|----------|
| `default` (`memory-for-ai`) | Standard server, no UI |
| `memory-for-ai-ui` | Server with the graph UI embedded (`--ui=true` works) |
...
```bash
# Standard server
nix run github:LonelyTraderBay/memory-for-ai
...
Or build a binary into `./result/bin/memory-for-ai`:

```bash
nix build github:LonelyTraderBay/memory-for-ai                          # standard
nix build github:LonelyTraderBay/memory-for-ai#memory-for-ai-ui   # with UI
```

Working in a clone? Use `.` in place of the flake URL, e.g. `nix run .#memory-for-ai-ui -- --ui=true`, or drop into a shell that puts the binary on `PATH` with `nix shell .#memory-for-ai-ui`.

> **Note:** launched by hand (not from an MCP client) the server exits as soon as `stdin` closes — that's normal MCP behaviour. Keep `stdin` open while testing the UI, e.g. `sleep infinity | memory-for-ai --ui=true --port=9749`. The `memory-for-ai-ui` package embeds the UI at build time; `nix run`'ing the standard `default` package with `--ui=true` will refuse to start the HTTP server.

### Install via Claude Code

```
You: "Install this MCP server: https://github.com/LonelyTraderBay/memory-for-ai"
```
...
# Binary at: build/c/memory-for-ai   (memory-for-ai.exe on Windows)
```
...
| OpenHands | Detected | `.openhands/mcp.json` | Shared `.agents/skills/memory-for-ai/SKILL.md` |
...
| Warp | Detected, skill only | UI, Warp Drive, or per invocation (manual) | Shared `~/.agents/skills/memory-for-ai/SKILL.md` |
...
| Grok Build | Detected | `$GROK_HOME/config.toml` | Owned `rules/memory-for-ai.md`, skill, three graph agents with named-server `mcpInheritance` and exact `server__tool` dispatcher ids; context hooks withheld because its passive hook events discard stdout |
...
| Amp | Detected | `~/.config/agents/skills/memory-for-ai/mcp.json` | Colocated skill + `~/.config/amp/AGENTS.md`; no plugin |
...
OpenClaw reinjects the `Codebase Knowledge Graph (memory-for-ai)` AGENTS
...
memory-for-ai cli query_graph --project my-project --query 'MATCH (f:Function) RETURN f.name LIMIT 5'

# Force human-readable progress without contaminating stdout.
memory-for-ai cli --progress index_repository --repo-path /path/to/repo
...
`Project`, `Package`, `Folder`, `File`, `Module`, `Class`, `Function`, `Method`, `Interface`, `Enum`, `Type`, `Route`, `Resource`, `BuildTarget`, `TestSuite`, `SecurityAudit`, `SecurityReport`, `SecurityAdvisory`

### Edge Types

`CONTAINS_PACKAGE`, `CONTAINS_FOLDER`, `CONTAINS_FILE`, `DEFINES`, `DEFINES_METHOD`, `IMPORTS`, `CALLS`, `CALL_REFERENCE`, `HTTP_CALLS`, `ASYNC_CALLS`, `IMPLEMENTS`, `HANDLES`, `USAGE`, `CONFIGURES`, `WRITES`, `MEMBER_OF`, `TESTS`, `TESTS_FILE`, `CONTAINS_TEST`, `BUILDS`, `RUNS_TESTS`, `DEPENDS_ON`, `SECURITY_SCANS`, `AUDITS`, `REPORTS`, `USES_TYPE`, `FILE_CHANGES_WITH`

See [Project graph extensions](docs/PROJECT_GRAPH.md) for the build/test,
dependency, and local security-evidence model.
...
```bash
memory-for-ai config list                          # show all settings
memory-for-ai config set auto_index true           # auto-index on session start
memory-for-ai config set auto_index_limit 50000    # max files for auto-index
memory-for-ai config set auto_watch false          # don't register background git watcher (default: true)
memory-for-ai config set watcher_enabled false     # stop the watcher thread entirely (default: true)
memory-for-ai config reset auto_index              # reset to default
```
...
| `MFA_CACHE_DIR` | `~/.cache/memory-for-ai` | Override the database storage directory. All project indexes and config are stored here. One account can use only one canonical cache root at a time; close active CBM sessions/commands before switching it. |
...
export MFA_CACHE_DIR=~/my-projects/cbm-data
```
...
```json
// ~/.config/memory-for-ai/config.json  (or $XDG_CONFIG_HOME/...)
...
SQLite databases stored at `~/.cache/memory-for-ai/`. Persists across restarts (WAL mode, ACID-safe). To reset: `rm -rf ~/.cache/memory-for-ai/`.
...
- **SLSA Level 3** — cryptographic build provenance generated by the trusted GitHub Actions build workflow; verify with `gh attestation verify <file> --repo LonelyTraderBay/memory-for-ai --signer-workflow LonelyTraderBay/memory-for-ai/.github/workflows/_build.yml`
