...
cat > "$ENV_PROBE" <<'EOF'
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "${HOME-}" "${MFA_CACHE_DIR-}" "${MFA_RUNTIME_DIR-}" >> "$CBM_FUZZ_ENV_PROBE"
...
if ! HOME="$CALLER_HOME" \
    MFA_CACHE_DIR="$CALLER_CACHE" \
    MFA_RUNTIME_DIR="$CALLER_RUNTIME" \
...
while IFS=$'\t' read -r child_home_raw child_cache_raw child_runtime_raw; do
    ...
    if [[ -z "$child_cache" || "$child_cache" == "$CALLER_CACHE_NORMALIZED" ]]; then
        echo "FAIL: security-fuzz exposed the caller MFA_CACHE_DIR to a fuzz target"
        exit 1
    fi
    if [[ -z "$child_runtime" || "$child_runtime" == "$CALLER_RUNTIME_NORMALIZED" ]]; then
        echo "FAIL: security-fuzz exposed the caller MFA_RUNTIME_DIR to a fuzz target"
        exit 1
    fi
