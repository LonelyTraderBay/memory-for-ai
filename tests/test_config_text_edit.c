...
#define CTE_BEGIN "<!-- memory-for-ai:start -->"
#define CTE_END "<!-- memory-for-ai:end -->"
...
TEST(config_text_owned_document_migrates_exact_releases_only) {
    char dir[CTE_PATH_CAP];
    char path[CTE_PATH_CAP];
    const char *current = "name: memory-for-ai\ntier: verify\n";
    const char *released[] = {"name: memory-for-ai\n", "name: memory-for-ai\nlegacy: 2\n"};
    const char *modified = "name: memory-for-ai\nuser-note: keep\n";
    ASSERT_EQ(cte_fixture(dir, sizeof(dir), path, sizeof(path)), 0);
