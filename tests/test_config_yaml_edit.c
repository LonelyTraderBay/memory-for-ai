...
static bool yaml_upsert_failed_unchanged(const char *path, const char *original) {
    const char *block = "    command: memory-for-ai\n";
    if (th_write_file(path, original) != 0 ||
        cbm_yaml_upsert_mapping_entry(path, "mcp_servers", "memory-for-ai", block) == 0) {
        return false;
    }
...
TEST(config_yaml_edit_rejects_stale_content_and_cleans_temp) {
    ...
    cbm_yaml_set_precommit_hook_for_testing(yaml_change_before_commit, &change);
    int result =
        cbm_yaml_upsert_mapping_entry(fixture.path, "mcp_servers", "memory-for-ai", block);
    cbm_yaml_set_precommit_hook_for_testing(NULL, NULL);
...
TEST(config_yaml_edit_absent_target_requires_safe_root_mapping) {
    ...
    ASSERT_EQ(cbm_yaml_upsert_mapping_entry(safe.path, "mcp_servers", "memory-for-ai",
...
TEST(config_yaml_edit_rejects_semantic_target_key_aliases) {
    ...
    ASSERT_EQ(cbm_yaml_upsert_mapping_entry(section.path, "mcp_servers", "memory-for-ai", block),
              -1);
    ...
    ASSERT_EQ(cbm_yaml_upsert_mapping_entry(entry.path, "mcp_servers", "memory-for-ai", block),
              -1);
...
TEST(config_yaml_edit_preserves_crlf_and_handles_no_final_newline) {
    ...
    ASSERT_EQ(cbm_yaml_upsert_mapping_entry(crlf.path, "mcp_servers", "memory-for-ai", block), 0);
    ...
    ASSERT_EQ(cbm_yaml_upsert_mapping_entry(no_final_newline.path, "mcp_servers", "memory-for-ai",
                                            block),
              0);
...
TEST(config_yaml_edit_hermes_mapping_lifecycle) {
    const char *initial = "# Hermes settings\n"
                          ...
                          "    command: other-mcp\n"
                          "theme: dark\n";
    const char *first_block = "    command: memory-for-ai\n"
                              "    args: [\"--stdio\"]\n";
    const char *replacement_block = "    command: /opt/memory-for-ai\n"
                                    "    args: [\"--stdio\"]\n";
    yaml_fixture_t fixture;
    ASSERT_EQ(yaml_fixture_init(&fixture, initial), 0);

    ASSERT_EQ(
        cbm_yaml_upsert_mapping_entry(fixture.path, "mcp_servers", "memory-for-ai", first_block),
        0);
    ...
    ASSERT_NOT_NULL(strstr(installed, "  memory-for-ai:\n"
                                      "    command: memory-for-ai\n"
                                      "    args: [\"--stdio\"]\n"));
    ASSERT_NOT_NULL(strstr(installed, "theme: dark\n"));
    ASSERT_EQ(yaml_count_occurrences(installed, "  memory-for-ai:\n"), 1);
    ...
    ASSERT_EQ(cbm_yaml_upsert_mapping_entry(fixture.path, "mcp_servers", "memory-for-ai",
                                            replacement_block),
              0);
    char *replaced = yaml_read_alloc(fixture.path);
    ASSERT_NOT_NULL(replaced);
    ASSERT_NOT_NULL(strstr(replaced, "    command: /opt/memory-for-ai\n"));
    ASSERT_NULL(strstr(replaced, "    command: memory-for-ai\n"));
    ASSERT_NOT_NULL(strstr(replaced, "  other:\n    command: other-mcp\n"));

    ASSERT_EQ(cbm_yaml_remove_mapping_entry(fixture.path, "mcp_servers", "memory-for-ai"), 0);
...
TEST(config_yaml_edit_hermes_creates_missing_section) {
    ...
    ASSERT_EQ(cbm_yaml_upsert_mapping_entry(fixture.path, "mcp_servers", "memory-for-ai", block),
              0);
    char *installed = yaml_read_alloc(fixture.path);
    ASSERT_NOT_NULL(installed);
...
TEST(config_yaml_edit_goose_extensions_preserve_siblings) {
    ...
    ASSERT_EQ(cbm_yaml_upsert_mapping_entry(fixture.path, "extensions", "memory-for-ai", block),
              0);
    char *installed = yaml_read_alloc(fixture.path);
    ASSERT_NOT_NULL(installed);
    ...
    ASSERT_EQ(cbm_yaml_remove_mapping_entry(fixture.path, "extensions", "memory-for-ai"), 0);
    char *removed = yaml_read_alloc(fixture.path);
    ASSERT_NOT_NULL(removed);
    ASSERT_STR_EQ(removed, initial);
...
TEST(config_yaml_edit_goose_accepts_empty_flow_mapping_in_sibling_issue1673) {
    ...
    ASSERT_EQ(cbm_yaml_upsert_owned_mapping_entry(fixture.path, "extensions", "memory-for-ai",
                                                  block),
...
static const char *const yaml_hermes_block = "    command: \"/opt/memory-for-ai\"\n";

static int yaml_hermes_upsert(const yaml_fixture_t *fixture) {
    return cbm_yaml_upsert_owned_mapping_entry(fixture->path, "mcp_servers", "memory-for-ai",
                                               yaml_hermes_block);
}
...
TEST(config_yaml_edit_hermes_accepts_empty_flow_sequence_value_issue1631) {
    ...
    ASSERT_NOT_NULL(strstr(after, "command: \"/opt/memory-for-ai\"\n"));
    free(after);
    th_cleanup(fixture.dir);
    PASS();
}
...
TEST(config_yaml_edit_repairs_prior_goose_block_without_name) {
    const char *initial = "extensions:\n"
                          "  memory-for-ai:\n"
                          "    type: stdio\n"
                          "    cmd: \"/old/place/memory-for-ai\"\n"
                          "    args: []\n"
                          "    enabled: true\n";
    const char *block = "    name: memory-for-ai\n"
                        "    type: stdio\n"
                        "    cmd: \"/opt/memory-for-ai\"\n"
                        "    args: []\n"
                        "    enabled: true\n";
    ...
    char *after = yaml_read_alloc(fixture.path);
    ASSERT_NOT_NULL(after);
    ASSERT_NOT_NULL(strstr(after, "name: memory-for-ai\n"));
    ASSERT_NOT_NULL(strstr(after, "cmd: \"/opt/memory-for-ai\"\n"));
...
TEST(config_yaml_edit_owned_agent_mapping_installs_idempotently_and_removes_exact_state) {
    ...
    for (size_t i = 0U; i < sizeof(cases) / sizeof(cases[0]); i++) {
        ...
        ASSERT_EQ(cbm_yaml_upsert_owned_mapping_entry(fixture.path, cases[i].section,
                                                      "memory-for-ai", cases[i].canonical),
                  CBM_YAML_IDENTITY_EDIT_OK);
        char *installed = yaml_read_alloc(fixture.path);
        ASSERT_NOT_NULL(installed);
...
TEST(config_yaml_edit_mapping_entry_is_explicit_managed_boundary) {
    ...
    ASSERT_EQ(
        cbm_yaml_upsert_mapping_entry(fixture.path, "mcp_servers", "memory-for-ai", managed), 0);
    char *after = yaml_read_alloc(fixture.path);
    ASSERT_NOT_NULL(after);
    ASSERT_NOT_NULL(strstr(after, "  memory-for-ai:\n"
                                  "    command: new-binary\n"
                                  "    args: []\n"));
...
TEST(config_yaml_edit_mapping_ambiguity_fails_unchanged) {
    const char *cases[] = {
        ...
        "mcp_servers: &shared\n  other:\n    command: other\n",
        "mcp_servers:\n  <<: *defaults\n  other:\n    command: other\n",
        "mcp_servers: {memory-for-ai: {command: memory-for-ai}}\n",
        "mcp_servers:\n  other:\n    description: >\n      folded text\n",
    };
    const char *block = "    command: memory-for-ai\n";
    ...
    ASSERT_EQ(cbm_yaml_upsert_mapping_entry(invalid_block.path, "mcp_servers", "memory-for-ai",
                                            "    command: &shared\n"),
              -1);
    ...
    ASSERT_EQ(cbm_yaml_upsert_mapping_entry(quoted_hash.path, "mcp_servers", "memory-for-ai",
...
TEST(config_yaml_edit_nested_sequence_preserves_crlf) {
    ...
    ASSERT_NOT_NULL(strstr(after, "    - id: \"memory-for-ai\"\r\n"));
#ifndef _WIN32
...
TEST(config_yaml_edit_nested_sequence_removes_only_exact_canonical_item) {
    ...
    char *removed = yaml_read_alloc(fixture.path);
    ASSERT_NOT_NULL(removed);
    ASSERT_NULL(strstr(removed, "id: \"memory-for-ai\""));
