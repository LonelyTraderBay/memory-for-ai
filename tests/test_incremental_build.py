...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="incremental-contract-", dir=build) as tmp:
        ...
        # Use public execution targets too: Windows must launch the .exe path.
        base[3] = "test"
        make("-DTEST_CONFIGURATION_CHANGED=1")
        # Both link recipes create response files, including with macOS system make.
        base[3] = "test-foundation"
        make("-DTEST_CONFIGURATION_CHANGED=1")
        assert times() == fourth, "foundation link unnecessarily recompiled shared objects"
        print("PASS: no-op 0 compiles/links; source/header 1 compile; flags 2 compiles; both executables correct")
