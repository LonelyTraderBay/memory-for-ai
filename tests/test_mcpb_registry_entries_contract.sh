...
def fresh_fixture():
    ...
    (fix / "checksums.txt").write_text(
        f"{SHA_B}  memory-for-ai-windows-amd64.zip\n"
        f"{SHA_B}  memory-for-ai-windows-amd64.mcpb\n"
        f"{SHA_A}  memory-for-ai-darwin-arm64.mcpb\n",
        encoding="utf-8",
    )
...
if len(mcpb) != 2:
    ...
    first = mcpb[0]
    wanted_url = ("https://github.com/LonelyTraderBay/memory-for-ai/releases/"
                  "download/v9.9.9/memory-for-ai-darwin-arm64.mcpb")
...
with (fix / "checksums.txt").open("a", encoding="utf-8") as handle:
    handle.write("deadbeef  memory-for-ai-darwin-amd64.mcpb\n")
result = run()
if result.returncode == 0:
